  ...
  if (typeof mxVertexHandler !== 'undefined') {
    ...
    this.addListener(mxEvent.FIRE_MOUSE_EVENT, mxUtils.bind(this, function (sender, evt) {
      if (evt.getProperty('eventName') == 'mouseDown') {
        ...
        //select node in cells
        var nodes = document.getElementsByClassName('k2-node-item');
        var node = me.evt.target;

        // 触发click监听，以便外部调用
        if (processDesign.graphClickListener) {
          processDesign.graphClickListener(me.evt);
        }

        // Virtual node
        var virtualNodeContainer = document.getElementsByClassName('virtual-node-container');
        if (node.className === 'k2-node-arrow' || node.className === 'virtual-node-container' || node.className === 'virtual-node-name') {
          var id = node.getAttribute('id'),
            data = FLOWSTOREDATA.getDataById(null, id),
            virtualNode,
            top = me.evt.clientY,
            width = node.offsetParent.clientWidth,
            left = me.evt.clientX;
          virtualNode = generateVirtualMatrixNodeEl(data.candidates.activityDestMatrix, id);

          if (!virtualNodeContainer[0]) {
            var container = document.createElement('div');

            container.className = 'virtual-node-container';
            container.style.width = width + 'px';
            container.style.position = 'fixed';
            container.style.top = top + 'px';
            container.style.left = (left + 32) + 'px';
            container.style.border = '1px solid #ccc';
            container.style.background = '#ffffff';
            container.innerHTML = virtualNode;
            document.body.appendChild(container);
          } else {
            virtualNodeContainer[0].style.width = width + 'px';
            virtualNodeContainer[0].style.top = top + 'px';
            virtualNodeContainer[0].style.left = (left + 32) + 'px';
            virtualNodeContainer[0].innerHTML = virtualNode;
            virtualNodeContainer[0].style.display = 'block';
          }
        } else {
          if (virtualNodeContainer[0]) {
            virtualNodeContainer[0].style.display = 'none';
          }
        }
        function generateVirtualMatrixNodeEl(item, nodeId) {
          if (item && item.length > 0) {
            ...
            for (var i = 0; i < item.length; i++) {
              ...
              //   if(item[i].virtActList === null) continue;
              if (item[i].virtActList && item[i].virtActList.length) {
                var ul = '<ul class="matrix-node-item-container" style="position:absolute;top:0;background:#ffffff;">';

                for (var j = 0; j < item[i].virtActList.length; j++) {
                  const uuid = processDesignUtils.uuid();
                  item[i].virtActList[j].activityInfo.virActId = uuid;
                  ul += '<li class="matrix-node-item" style="color:#000000;margin:0;list-style: none;  overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + item[i].virtActList[j].activityInfo.displayName[window.flowI18n.lang] + '" data-node-id="' + nodeId + '" id="' + item[i].id + '?' + uuid+ '">' + item[i].virtActList[j].activityInfo.displayName[window.flowI18n.lang] + '</li>';
                }
                ul += '</ul>';
              }

              //修改场景规则名称显示问题
              var matrixName='默认场景规则说明'+(i+1);
              if(item[i].description && item[i].description[window.flowI18n.lang]){
                matrixName = item[i].description[window.flowI18n.lang];
              }

              if (item[i].virtActList && item[i].virtActList.length > 0) {
                returnValue += '<div class="matrix-node-name" data-node-id="' + nodeId + '" id="' + item[i].id + '"  style="position:relative;"><span class="matrix-node-arrow" title="' + matrixName + '">' + matrixName + '</span>' + ul + '</div>';
              } else {
                returnValue += '<div class="matrix-node-name" data-node-id="' + nodeId + '" id="' + item[i].id + '"  style="position:relative;"><span title="' + matrixName + '">' + matrixName + '</span></div>';
              }
            }
      ...
      } else if (evt.getProperty('eventName') == 'mouseMove') {
        var me = evt.getProperty('event');
        var state = me.getState();
        ...
        var node = me.evt.target;
        // 触发move监听，以便外部调用
        if (processDesign.graphMoveListener) {
          processDesign.graphMoveListener(node);
        }
      }
    }));
...
Graph.prototype.init = function (container) {
  ...
  this.cellRenderer.initializeLabel = function (state, shape) {
    ...
    /* 2021-0301 zzy 添加双击事件 */
    mxEvent.addListener(shape.node, 'dblclick', function (evt) {
      var eventsOporates = FLOWINSTANCE.editorUi.eventsOporates();
      var cell = FLOWINSTANCE.editorUi.editor.graph.getSelectionCells()[0];
      if (cell != null) {
        // 流程挂起节点和线不允许编辑
        ...
        // 流程开始，流程结束 不允许编辑
        if (cell.nodeType !== processDesignConstant.NODE_TYPE.START && cell.nodeType !== processDesignConstant.NODE_TYPE.END) {
          // 打开节点的属性面板
          eventsOporates.attribute(cell);
        }
      }
    });
  };
...
if (typeof mxVertexHandler != 'undefined') {
  (function () {
    ...
    mxVertexHandler.prototype.mouseMove = function (sender, me) {
      ...
      //
      if (FLOWINSTANCE && FLOWINSTANCE.getSidebarStatus()) {
        if (document.getElementsByClassName('geDiagramContainer') && document.getElementsByClassName('geDiagramContainer')[0]) {
          document.getElementsByClassName('geDiagramContainer')[0].setAttribute('class', 'geDiagramContainer');
        }
      } else {
        if (document.getElementsByClassName('geDiagramContainer') && document.getElementsByClassName('geDiagramContainer')[0]) {
          if(window.NODE_TYPE_STYLE){
            document.getElementsByClassName('geDiagramContainer')[0].setAttribute('class', 'geDiagramContainer collapse');
          }else{
            document.getElementsByClassName('geDiagramContainer')[0].setAttribute('class', 'geDiagramContainer');
          }
        }
