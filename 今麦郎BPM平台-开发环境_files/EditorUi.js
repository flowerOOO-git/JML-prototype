...
EditorUi = function (editor, container, lightbox) {
  ...
  if (this.menus != null) {
    graph.popupMenuHandler.factoryMethod = mxUtils.bind(this, function (menu, cell, evt) {
      this.createCumstomerPopupMenu(menu, cell, evt);
      this.monitorEmptyEditPopupMenu(menu, cell, evt);
    });
  }
   ...
   *画布菜单事件
   * */
  this.monitorEmptyEditPopupMenu = function (menu, cell, evt) {
    if (graph.isSelectionEmpty() && !window.NODE_TYPE_STYLE) {
      /* 2021-10-20 取消 撤销键 */
      // this.menus.addMenuItems(menu, ['undo', 'redo', '-', 'pasteHere'], null, evt);
      // yuminghao 撤销功能 Ctrl+Z 重做功能 Ctrl+y 画布菜单屏蔽
      this.menus.addMenuItems(menu, [ '-', 'pasteHere'], null, evt);
    }
  };

  // update cells
  this.updateCell = function (nodeType, id, data) {
    var graph = this.editor.graph,
    ...
    try {
      if (nodeType !== processDesignConstant.NODE_TYPE.LINE && nodeType !== processDesignConstant.NODE_TYPE.SELF_LINE && nodeType !== processDesignConstant.NODE_TYPE.FROM_LOOP_NODE_LINE) {
        // if (nodeType === processDesignConstant.NODE_TYPE.CLIENT || nodeType === processDesignConstant.NODE_TYPE.SERVICE || nodeType === processDesignConstant.NODE_TYPE.SELF_LOOP || nodeType === processDesignConstant.NODE_TYPE.TOA || nodeType === processDesignConstant.NODE_TYPE.SUB_PROCESS) {

        var nodeItems = [];
        if (nodeType === processDesignConstant.NODE_TYPE.SERVICE) {
          // nodeItems = [{'name': 'service'}];
        } else if (nodeType === processDesignConstant.NODE_TYPE.CLIENT) {
          nodeItems = [{'name': 'client'}];
        } else if (nodeType === processDesignConstant.NODE_TYPE.SUB_PROCESS) {
          nodeItems = [{'name': 'ipcEvent'}];
        }

        if (data && data.activityClientEvent && data.activityClientEvent.clientEventName) {
          if (nodeItems.length === 0) {
            nodeItems.push({
              name: data.activityClientEvent.clientEventName
            });
          } else {
            nodeItems[0].name = data.activityClientEvent.clientEventName;
          }
        }

        if (data && data.activityServerEvents) {
          for (var i = 0; i < data.activityServerEvents.length; i++) {
            if (data.activityServerEvents[i].serverEventOrder === 'Before') {
              nodeItems.unshift({name: data.activityServerEvents[i].eventName});
            } else if (data.activityServerEvents[i].serverEventOrder === 'After') {
              nodeItems.push({name: data.activityServerEvents[i].eventName});
            } else {
              nodeItems.push({name: data.activityServerEvents[i].eventName});
            }
          }
        ...
        if (data.activityCycle && ((data.virtActs && data.virtActs.length > 0) || (data.matrixs && data.matrixs.length > 0))) {
          isHasVirAct = true;

          if (data.matrixs && data.matrixs.length > 0) {
            isHasVirAct = false;

            for (var i = 0; i < data.matrixs.length; i++) {
              if (data.matrixs[i].matrixVirtActs && data.matrixs[i].matrixVirtActs.length > 0) {//矩阵审批的场景中虚拟节点有可能为空,增加校验
                isHasVirAct = true;
                break;
              }
            }
        ...
        var nodeName =  data.activityName;
        nodeName =  window.flowI18n.handerlNodeLabel(data.activityName);
        if (isHasVirAct) {
          currentCell.value = '<p class="k2-node-title"><span style="background:transparent url(' + processDesignConstant.NODE_ICON[nodeType] + ') no-repeat left center;">' + nodeName + '</span><span class="k2-node-arrow" id="' + data.id + '"></span></p>' + processDesignUtils.generateElement(nodeItems);
        } else {
          currentCell.value = '<p class="k2-node-title"><span style="background:transparent url(' + processDesignConstant.NODE_ICON[nodeType] + ') no-repeat left center;">' + nodeName + '</span></p>' + processDesignUtils.generateElement(nodeItems);
        }
        // model中的nodeName和业务数据中的activityName保持一致
        currentCell.nodeName = nodeName;
      } else {
        currentCell.value = data.lineLabel;
      }
      // 根据审批行为生成对应的线
      if (data.autoGenerateLine && data.activityOutComes && data.activityOutComes.length > 0) {
        for (var j = 0; j < data.activityOutComes.length; j++) {
          // 检查model中是否已经存在对应生成的线
          var isNewOutCome = true;
          for (var key in graph.model.cells) {
            if (graph.model.cells.hasOwnProperty(key)) {
              if (data.activityOutComes[j].id && data.activityOutComes[j].id === graph.model.cells[key].outComeId) {
                isNewOutCome = false;
                break;
              }
            }
          ...
          if (isNewOutCome) {
            ...
            var edge = graph.insertEdge(parent, null, data.activityOutComes[j].outComeName, currentCell, null, lineStyle);
            var geo = edge.geometry;
            var pt = new mxPoint(currentCell.geometry.x + (j + 1) * 30, currentCell.geometry.y + currentCell.geometry.height + 40);
            geo.points = [];
            geo.points.push(pt);
            geo.targetPoint = pt;
            edge.outComeId = data.activityOutComes[j].id;
          
            var edgeData = {
              id: edge.id,
              nodeType: edge.nodeType,
              lineLabel: data.activityOutComes[j].outComeName,
              lineName: data.activityOutComes[j].outComeName,
              actionResult: data.activityOutComes[j].outComeName,
              displayName: data.activityOutComes[j].displayName,
              dataFields: data.dataFields,
            };
            window.FLOWSTOREDATA.setDataById(edge.nodeType, edge.id, edgeData);
          }
        }
      }
      graph.view.refresh();
    ...
    window.FLOWSTOREDATA.setDataById(nodeType, id, data);
  };
  // 根据线的id获取线的起始点
  this.getSourceCell = function (id) {
    var graph = this.editor.graph,
  ...
  // 根据线的id获取线的结束节点
  this.getTargetCell = function (id) {
    var graph = this.editor.graph,
      cells = graph.model.cells;
  ...
  //
  this.getSourceCellId = function (id) {
    var graph = this.editor.graph,
      cells = graph.model.cells;
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 118 : 260;
...
EditorUi.prototype.init = function () {
  ...
  //
  var designBarBtn = mxUtils.button('设计', mxUtils.bind(this, function () {
    this.diagramContainer.style.overflow = 'auto';

    var geSidebar = document.getElementsByClassName('geSidebar')[0];

    if (geSidebar) {
      for (var i = 0; i < geSidebar.children.length; i++) {
        geSidebar.children[i].style.pointerEvents = 'fill';
      }
    }
    ...
    if (container) {
      this.diagramContainer.removeChild(container);
    }
  }));

  var sourceBarBtn = mxUtils.button('源码', mxUtils.bind(this, function () {
    ...
    if (geSidebar) {
      for (var i = 0; i < geSidebar.children.length; i++) {
        geSidebar.children[i].style.pointerEvents = 'none';
      }
    }

    var encoder = new mxCodec();
    var node = encoder.encode(graph.getModel());
    var height = this.diagramContainer.clientHeight;
    var scrollHeight = this.diagramContainer.scrollTop;
    var container = document.getElementById('sourceContainer');

    if (!container) {
      var sourceContainer = document.createElement('div');
      sourceContainer.id = 'sourceContainer';
      sourceContainer.style.cssText = 'position:absolute;left:0;top:' + scrollHeight + 'px;right:0;bottom:0;width:100%;z-index:10000;background:#fff;overflow:auto;height:' + height + 'px';

      var pre = document.createElement('div');
      pre.style.padding = '10px';
      pre.style.borderTop = '1px solid #e5e5e5';
      pre.innerHTML = mxUtils.htmlEntities(mxUtils.getPrettyXml(node), false).replace(/\n/g, '<br>').replace(/ /g, '&nbsp;');

      sourceContainer.appendChild(pre);
      this.diagramContainer.appendChild(sourceContainer);
    } else {
      container.style.height = height + 'px';
      container.children[0] = mxUtils.htmlEntities(mxUtils.getPrettyXml(node), false).replace(/\n/g, '<br>').replace(/ /g, '&nbsp;');
    }
  }));

  sourceBarBtn.className = 'source-bar-btn';
  designBarBtn.className = 'design-bar-btn';

  this.footerContainer.appendChild(designBarBtn);
  this.footerContainer.appendChild(sourceBarBtn);
  //
  this.diagramContainer.addEventListener('scroll', function () {
    if (document.getElementsByClassName('virtual-node-container')[0]) {
      document.getElementsByClassName('virtual-node-container')[0].style.display = 'none';
    }
  });
};
/**
...
EditorUi.prototype.updatePasteActionStates = function () {
  ...
  if(pasteHere){
    pasteHere.label = window.flowI18n.$t('flowDesigner.menuBtn.pasteHere');
  }
...
EditorUi.prototype.initCanvas = function () {
  ...
  /**
   * Returns the padding for pages in page view with scrollbars. 去掉画布初始化时上下左右四个方向的填充
   */
...
EditorUi.prototype.redo = function () {
  try {
    var graph = this.editor.graph;
    // yuminghao 重做功能 Ctrl+y 判断是否能重做 
    // isredo变量在Datstore中控制
    if(!graph.isredo)return;
...
EditorUi.prototype.undo = function () {
  try {
    var graph = this.editor.graph;
    // zzy 2021-03-08 添加判断是否能够继续撤销
    // yuminghao 撤销功能 Ctrl+Z 判断是否能撤销
    // isredo变量在Datstore中控制
    if(!graph.isundo)return;
...
EditorUi.prototype.addUndoListener = function () {
  ...
  var undoListener = mxUtils.bind(this, function (evt) {
...
EditorUi.prototype.createDivs = function () {
  this.menubarContainer = this.createDiv('geMenubarContainer fw-head-nav');
  this.toolbarContainer = this.createDiv('geToolbarContainer');
  this.sidebarContainer = this.createDiv('geSidebarContainer');
  this.formatContainer = this.createDiv('geSidebarContainer geFormatContainer');
  this.diagramContainer = this.createDiv('geDiagramContainer');
  this.footerContainer = this.createDiv('geFooterContainer');
  this.homeContainer = this.createDiv('geHomeContainer'); // 欢迎页面
  this.hsplit = this.createDiv('geHsplit');
  this.hsplit.setAttribute('title', mxResources.get('collapseExpand'));

  // Sets static style for containers
  this.menubarContainer.style.top = '16px';
