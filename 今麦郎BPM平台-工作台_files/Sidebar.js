...
Sidebar.prototype.appDataSources = window.appDataSources;
/**
 * Sidebar data
 */
const appDataSources = {
  ...
  paramsData: {
    leftTree: {
      _WORKFLOWS_LEFT_TREES_ROOT_NAME: '工作节点',
      _WORKFLOWS_LEFT_TREES_NODES: {
        // 用户节点
        'workflow_node_6': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.CLIENT],
          type: processDesignConstant.NODE_TYPE.CLIENT,
          shape: processDesignConstant.NODE_SHAPE.SCROLL,
          default: [
            {
              'name': 'client'
            }
        ...
        // 逐级审批（循环节点）
        'workflow_node_10': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.SELF_LOOP],
          type: processDesignConstant.NODE_TYPE.SELF_LOOP,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        },
        // 矩阵
        'workflow_node_8': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.TOA],
          type: processDesignConstant.NODE_TYPE.TOA,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        },
        // 服务节点
        'workflow_node_9': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.SERVICE],
          type: processDesignConstant.NODE_TYPE.SERVICE,
          shape: processDesignConstant.NODE_SHAPE.SCROLL,
          default: [
            {
              'name': 'service'
        ...
        //网关
        'workflow_node_13': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.GATEWAY],
          type: processDesignConstant.NODE_TYPE.GATEWAY,
          shape: processDesignConstant.NODE_SHAPE.JUDGE
        },
        // 子流程
        'workflow_node_7': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.SUB_PROCESS],
          type: processDesignConstant.NODE_TYPE.SUB_PROCESS,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        },
        // 迭代节点类型
        /* 'workflow_node_11': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.ITERATION],
          type: processDesignConstant.NODE_TYPE.ITERATION,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        }, */
        // 瀑布节点类型
        /* 'workflow_node_12': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.WATERFALL],
          type: processDesignConstant.NODE_TYPE.WATERFALL,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        } */
        // 总分总开始
        'workflow_node_16': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.TOTAL_GROSS_START],
          type: processDesignConstant.NODE_TYPE.TOTAL_GROSS_START,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        }, 
        // 总分总结束
        'workflow_node_17': {
          activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.TOTAL_GROSS_END],
          type: processDesignConstant.NODE_TYPE.TOTAL_GROSS_END,
          shape: processDesignConstant.NODE_SHAPE.SCROLL
        } 
      },
      _WORKFLOWS_LEFT_TREES_LINES: {
        'workflow_line_0': {
          activityName: '圆形连接线',
          type: processDesignConstant.NODE_TYPE.LINE
        }
      }
    }
  }
...
Sidebar.prototype.init = function () {
  if(!this.appDataSources && !window.appDataSources){
    this.appDataSources = appDataSources;
  }else if(window.appDataSources){
    this.appDataSources =  window.appDataSources;
  }
  this.addNodePalette(true);
};
 ...
 */
Sidebar.prototype.thumbWidth = 226;
 ...
 */
Sidebar.prototype.thumbBorder = 0;
 ...
 * add node palette
 */
Sidebar.prototype.addNodePalette = function (expand) {
  var fns = [];
  // 如果是flowable引擎，需要新增两个节点（网关节点和end节点）隐藏 总分总开始，总分总结束
  if (processDesign.engineType === processDesignConstant.ENGINE_TYPE.FLOWABLE) {
    this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_13'] = {
      activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.GATEWAY],
      type: processDesignConstant.NODE_TYPE.GATEWAY,
      shape: processDesignConstant.NODE_SHAPE.JUDGE
    };
    // 子流程
    // if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_7']) {
    //   delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_7'];
    // }
    // 逐级审批
    // if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_10']) {
    //   delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_10'];
    // }
    //总分总开始
    if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_16']) {
      delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_16'];
    }
    //总分总结束
    if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_17']) {
      delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_17'];
    }
  } else {
    // 子流程
    this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_7'] = {
      activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.SUB_PROCESS],
      type: processDesignConstant.NODE_TYPE.SUB_PROCESS,
      shape: processDesignConstant.NODE_SHAPE.SCROLL
    };
    // 逐级审批（循环节点）
    this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_10'] = {
      activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.SELF_LOOP],
      type: processDesignConstant.NODE_TYPE.SELF_LOOP,
      shape: processDesignConstant.NODE_SHAPE.SCROLL
    };
    // 总分总开始
    this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_16'] = {
      activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.TOTAL_GROSS_START],
      type: processDesignConstant.NODE_TYPE.TOTAL_GROSS_START,
      shape: processDesignConstant.NODE_SHAPE.SCROLL
    };
    // 总分总结束
    this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_17'] = {
      activityName: processDesignConstant.NODE_NAME[processDesignConstant.NODE_TYPE.TOTAL_GROSS_END],
      type: processDesignConstant.NODE_TYPE.TOTAL_GROSS_END,
      shape: processDesignConstant.NODE_SHAPE.SCROLL
    };
    // 如果是K2引擎则不需要 网关节点(13)和 flowable引擎下的 end节点(14)
    // 网关节点
    if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_13']) {
      delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_13'];
    }
    // flowable引擎下的 结束节点
    if (this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_14']) {
      delete this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_NODES['workflow_node_14'];
    }
  }
  this.initWorkFlowNodes(fns, this.appDataSources.paramsData.leftTree);
  this.addPaletteFunctions('misc', this.appDataSources.paramsData.leftTree._WORKFLOWS_LEFT_TREES_ROOT_NAME, (expand != null) ? expand : true, fns);
};
/**
 * Adds the general palette to the sidebar.
 */
...
Sidebar.prototype.initWorkFlowNodes = function (fns, data) {
  var i = 0, value = null, style,width;
  for (var key in data._WORKFLOWS_LEFT_TREES_NODES) {
    var node = data._WORKFLOWS_LEFT_TREES_NODES[key];
    if (node.shape !== processDesignConstant.NODE_SHAPE.STANDARD) {
      var nodeName = node.activityName;
      nodeName =  window.flowI18n.handerlNodeLabel(nodeName);
      
      if (node.shape === processDesignConstant.NODE_SHAPE.JUDGE && processDesign.engineType === processDesignConstant.ENGINE_TYPE.FLOWABLE) {
        // fns[i] = this.createVertexTemplateEntry(style, 150, 86, value, node.activityName, null, null, 'label image icon symbol', node.type, node.activityName);
        value = '<p>' + nodeName + '</p>';
        // yuminghao 复写部分
        // 如果是流程图的页面 并且是 客户端 自循环 矩阵节点
        width = ((node.type === 6 || node.type === 10 || node.type === 8) && (window.location.href.includes('flowDesigner'))) ? 142 : 110;
        fns[i] = this.createVertexTemplateEntry('html=1;whiteSpace=wrap;aspect=fixed;shape=isoRectangle;', 150, width, value, node.activityName, null, null, 'rectangle rect box iso isometric', node.type, node.activityName);
      } else {
        var styleValue = 'background:'+ processColor.$nblColor01 +';margin:0;padding:0;text-align: center;height:24px;line-height: 24px;';
        value = '<p class="k2-node-title"><span style="background:transparent url('+ processDesignConstant.NODE_ICON[node.type] +') no-repeat left center;">' + nodeName + '</span></p>' + processDesignUtils.generateElement(node.default);
        // style = 'rotatable=0;verticalAlign=top;align=left;overflow=fill;fontSize=16;strokeColor=#bababa;color='+ processColor.$nblColor05 +';fontFamily=Microsoft YaHei;html=1;';
        // 节点
        if(node.type === 6 || node.type === 10 || node.type === 8){
          width = 142;
        }else{
          if(window.location.href.includes('flowDesigner')){
            width = 110;
          }else{
            width = 95;
          }
        ...
        fns[i] = this.createVertexTemplateEntry(nodeStyle, 150, width, value, node.activityName, null, null, 'label image icon symbol', node.type, node.activityName);
      }
    } else {
      style = 'rotatable=0;whiteSpace=nowrap;overflow=hidden;fillColor='+ processColor.$nblColor01 +';fontSize=16;strokeColor=transparent;fontColor='+ processColor.$nblColor05 +';fontFamily=Microsoft YaHei;right;html=1;';
      //
      if (node.type === processDesignConstant.NODE_TYPE.FLOWABLE_END && processDesign.engineType === processDesignConstant.ENGINE_TYPE.FLOWABLE) {
        value = '<p style="margin:0;padding:0 0 0 14px;background:url('+ processDesignConstant.NODE_ICON[node.type] +') no-repeat left center;background-size: 12px 12px;">' + nodeName + '</p>';
        fns[i] = this.createVertexTemplateEntry(style, 150, 40, value, node.activityName, null, null, 'label image icon symbol', node.type, node.activityName);
      } else {
        value = '<p style="margin:0;padding:0 0 0 14px;background:url('+ processDesignConstant.NODE_ICON[node.type] +') no-repeat left center;background-size: 10px 12px;">' + nodeName + '</p>';
        fns[i] = this.createVertexTemplateEntry(style, 150, 40, value, node.activityName, null, null, 'label image icon symbol', node.type, node.activityName);
      }
    }
...
Sidebar.prototype.initWorkFlowLines = function (fns, data, lineValue) {
  var i = fns.length;

  for (var key in data._WORKFLOWS_LEFT_TREES_LINES) {
    var node = data._WORKFLOWS_LEFT_TREES_LINES[key];

    fns[i] = this.createEdgeTemplateEntry(lineValue, 50, 50, node.name, node.name, null, null, 'label image icon symbol', node.type, node.name);
...
Sidebar.prototype.addUmlPalette = function (expand) {
  ...
  for (var key in this.appDataSources.paramsData.uml._WORKFLOWS_LEFT_TREES_NODES) {
    var node = this.appDataSources.paramsData.uml._WORKFLOWS_LEFT_TREES_NODES[key];
    var nodeHtml = '<p class="k2-node-title">' + node.name + '</p>';
    var cellType = node.type;

    fns[i] = this.createVertexTemplateEntry('verticalAlign=top;align=left;overflow=fill;fontSize=12;strokeColor=#ababab;fontFamily=Helvetica;html=1;', 100, 55, nodeHtml,
      node.name,
      null, null, 'uml static class',
      cellType, node.name);
    i++;
  }

  var lineValue = 'edgeStyle=elbowEdgeStyle;elbow=horizontal;endArrow=classic;html=1;';
  this.initWorkFlowLines(fns, this.appDataSources.paramsData.uml, lineValue);

  this.addPaletteFunctions('uml', this.appDataSources.paramsData.uml._WORKFLOWS_LEFT_TREES_ROOT_NAME, expand || false, fns);
};
...
Sidebar.prototype.createThumb = function (cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight) {
  ...
  // 重新生成左边缩略图元素，覆盖默认的node;
  node = document.createElement('span');
  
  // 向导id
  node.setAttribute('id','node-' + cells[0].nodeType + '-deiver');
  node.setAttribute('class', processDesignConstant.NODE_THUMB_CLASS_NAME[cells[0].nodeType]+'');
  ...
  /*node.style.backgroundImage = 'url(' + processDesignConstant.NODE_THUMB_ICON[cells[0].nodeType] + ')';
    node.style.backgroundPosition = 'left 0px';
    node.style.backgroundRepeat = 'no-repeat';
    node.style.backgroundSize = '28px 38px';
  ...
  // if (cells[0].nodeType === processDesignConstant.NODE_TYPE.CLIENT || cells[0].nodeType === processDesignConstant.NODE_TYPE.SERVICE) {
  //   node.innerHTML = window.NODE_TYPE_TITLE ? window.NODE_TYPE_TITLE[cells[0].nodeType] : title + '事件';
  // } else {
  //   node.innerHTML = title + '节点';
  // }

  // yuminghao复写  除了逐级审批，其他后面都加上节点
  // 标准流程设计器的 节点
  if(window.NODE_TYPE_TITLE && (cells[0].nodeType === processDesignConstant.NODE_TYPE.CLIENT || cells[0].nodeType === processDesignConstant.NODE_TYPE.SERVICE)){
    node.innerHTML =  window.NODE_TYPE_TITLE[cells[0].nodeType] ;
  }else{
    if(cells[0].nodeType !== processDesignConstant.NODE_TYPE.SELF_LOOP){
      node.innerHTML = title  + window.flowI18n.$t('flowDesigner.node');
    }else{
...
Sidebar.prototype.createItem = function (cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
  ...
  elt.setAttribute('href', 'javascript:void(0);');
  // 服务端 || 网关 || 并行开始 || 并行结束
  if(cells[0].nodeType === 9 || cells[0].nodeType === 13 || cells[0].nodeType === 16 || cells[0].nodeType === 17){
    elt.className = 'geItem getItem-server';
    // 子流程 
  }else if(cells[0].nodeType === 7){
    elt.className = 'geItem getItem-subprocess';
    // 循环节点
  }else if(cells[0].nodeType === 8){
    elt.className = 'geItem getItem-loop';
    // 逐级节点
  }else if(cells[0].nodeType === 10){
    elt.className = 'geItem getItem-stepByStep';
  }else{
...
Sidebar.prototype.createVertexTemplateEntry = function (style, width, height, value, title, showLabel, showTitle, tags, nodeType, nodeName) {
  tags = (tags != null && tags.length > 0) ? tags : title.toLowerCase();

  return this.addEntry(tags, mxUtils.bind(this, function () {
    return this.createVertexTemplate(style, width, height, value, title, showLabel, showTitle, true, nodeType, nodeName);
  }));
};
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function (style, width, height, value, title, showLabel, showTitle, allowCellsInserted, nodeType, nodeName) {
  var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style, nodeType, nodeName)];
  cells[0].vertex = true;
  cells[0].nodeType = nodeType;
  cells[0].nodeName = nodeName;
 ...
 *
 */
Sidebar.prototype.createEdgeTemplateEntry = function (style, width, height, value, title, showLabel, tags, allowCellsInserted, nodeType, nodeName) {
  tags = (tags != null && tags.length > 0) ? tags : title.toLowerCase();
  return this.addEntry(tags, mxUtils.bind(this, function () {
    return this.createEdgeTemplate(style, width, height, value, title, showLabel, allowCellsInserted, nodeType, nodeName);
  }));
};
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createEdgeTemplate = function (style, width, height, value, title, showLabel, allowCellsInserted, nodeType, nodeName) {
  var cell = new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style, nodeType, nodeName);
