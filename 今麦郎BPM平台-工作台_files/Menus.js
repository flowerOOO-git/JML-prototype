...
// 菜单下拉框是否显示
Menus.prototype.isShowNavMenu = false;
...
Menus.prototype.init = function () {
  ...
  this.put('edit', new Menu(mxUtils.bind(this, function (menu, parent) {
    /* 2021-10-20 注释撤销 this.addMenuItems(menu, ['undo', */
    this.addMenuItems(menu, ['redo', '-', 'undo','cut', 'copy', 'paste', 'delete', '-', 'duplicate', '-',
 ...
 * Creates the keyboard event handler for the current graph and history.
 */
Menus.prototype.createNavMenu = function (container, isNavMenu) {
  var menubar = new Menubar(this.editorUi, container);
  var menus = isNavMenu ? this.navMenuItems : this.menuItems;

  for (var i = 0; i < menus.length; i++) {
    var elt = menubar.addButton(menus[i].value, this.MENU_CLASS_NAME[menus[i].name], mxUtils.bind(this, this.handlers[menus[i].name]));
  }

  var that = this;
  document.body.addEventListener('click', function () {
    if (that.editorUi.navMenu.container) {
      that.editorUi.navMenu.container.style.display = 'none';
    }
    that.isShowNavMenu = false;
  }, false);
...
/**
 * Adds a button to this menubar.
 */
Menubar.prototype.addButton = function (label, className, funct) {
  var elt = document.createElement('button');
  elt.setAttribute('data-title', label);
  ...
  mxUtils.write(elt, label);
  if (label === '保存' || label === '流程属性' || label === '更多') {
    elt.innerText = '';
  }
