...
Blockly.Json.getWorkspace = function (workspace) {
    var width = Blockly.svgSize(workspace.getParentSvg()).width;
    var json = {};
    json.block = [];
...
Blockly.Json.blockToDom_ = function (block) {
    var element = {};
    if (block.num) {
      element.num = block.num;
    }
    element.type = block.type;
    element.id = block.id;
    if (block.getMutationParams) {
        // Custom data for an advanced block.
        var mutation = block.getMutationParams();
        if (mutation) {
...
Blockly.Json.domToBlock = function (workspace, jsonBlock, opt_reuseBlock) {
  ...
  if (jsonBlock.num) {
    if (jsonBlock.num > Blockly.Drools.count) {
      Blockly.Drools.count = jsonBlock.num;
    }
    block.num = jsonBlock.num;
  } else if (jsonBlock.id) {
      if (!isNaN(parseInt(jsonBlock.id))) {
        var num = parseInt(jsonBlock.id);
        jsonBlock.num = num;
        if (jsonBlock.num > Blockly.Drools.count) {
          Blockly.Drools.count = jsonBlock.num;
        }
